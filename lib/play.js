  ...
  exports.sound = function(file, callback){
    ...
    childD.addListener('exit', function (code, signal) {
      if(code == null || signal != null) {
        sys.puts('couldnt play, had an error ' + '[code: '+ code + '] ' + '[signal: ' + signal + ']');
      }
      if(err.search("Error: AudioFileOpen failed (-43)")){
        sys.puts( 'ERROR: Could not find file '.red + '=>'.yellow + file.magenta);
      } 
      else{
